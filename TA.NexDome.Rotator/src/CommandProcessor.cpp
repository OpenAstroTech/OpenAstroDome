...
void CommandProcessor::HandleCommand(const Command &command) const
{
	...
	{
		if (command.Verb == "AR")
			return HandleAR(command); // Read acceleration ramp time (ms)
		if (command.Verb == "AW")
			return HandleAW(command); // Write acceleration ramp time (ms)
		if (command.Verb == "DR")
			return HandleDR(command); // Read dead zone (rotator only)
		if (command.Verb == "DW")
			return HandleDW(command); // Write dead zone (rotator only)
		if (command.Verb == "FR")
			return HandleFR(command); // Read firmware version
		if (command.Verb == "GA")
			return HandleGA(command); // Goto Azimuth (rotator only)
		if (command.Verb == "GH")
			return HandleGH(command); // Goto Home Sensor (rotator only)
		if (command.Verb == "GS")
			return HandleGS(command); // Goto step position (rotator only)
		if (command.Verb == "HR")
			return HandleHR(command); // Home position Read (rotator only)
		if (command.Verb == "HW")
			return HandleHW(command); // Home position Write (rotator only)
		if (command.Verb == "SW")
			return HandleSW(command); // Stop motor (emergency stop)
		if (command.Verb == "PR")
			return HandlePR(command); // Position read
		if (command.Verb == "PW")
			return HandlePW(command); // Position write (sync)
		if (command.Verb == "RR")
			return HandleRR(command); // Range Read (circumference in steps)
		if (command.Verb == "RW")
			return HandleRW(command); // Range Write (circumference in steps)
		if (command.Verb == "SR")
			return HandleSR(command); // Status Request
		if (command.Verb == "VR")
			return HandleVR(command); // Velocity Read (motor speed, steps/s)
		if (command.Verb == "VW")
			return HandleVW(command); // Velocity Write (motor speed steps/s)
		if (command.Verb == "ZD")
			return HandleZD(command); // Reset to factory settings (load defaults).
		if (command.Verb == "ZR")
			return HandleZR(command); // Load settings from persistent storage
		if (command.Verb == "ZW")
			return HandleZW(command); // Write settings to persistent storage
		if (command.Verb == "ZZ")
			return HandleZZ(command); // Reboot immediately
	}
}
